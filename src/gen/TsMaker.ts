...
export class TsMaker {
    ...
    private codeFromFunctionExpressionInternal(funcName: string, isStatic: boolean, kind: string, accessibility: Accessibility, ast: FunctionExpression): string {
        ...
        if(this.crtClass) {
            ...
            if(this.crtFunc) {
                ...
                let funcInfo = this.crtClass.functionMap[funcKey];
    ...
    private codeFromIdentifier(ast: Identifier): string {
        ...
        if(this.addThisFlag && null != this.crtClass) {
            ...
            (!(ast as any).__parent || this.parentNoThis.indexOf((ast as any).__parent.type) < 0 && ((ast as any).__parent.type != AST_NODE_TYPES.MemberExpression || (ast as any).__memberExp_is_object || (ast as any).__memberExp_is_computed_property)) && 
            (!this.crtFunc || this.crtFunc.params.indexOf(str) < 0 && this.crtFunc.localVars.indexOf(str) < 0)) {
                let minfo = this.getMemberInfo(ast, this.crtClass, str);
                if(minfo) {
                    if(minfo.static) {
    ...
    private codeFromImportDeclaration(ast: ImportDeclaration): string {
        ...
        if(asModuleFormular) {
            ...
            if(!this.option.module) {
                ...
                let sourceStr = ' = ' + sourceValue.replace(/\//g, '.');
    ...
    private codeFromProperty(ast: Property): string {
        ...
        let keyStr = this.codeFromAST(ast.key);
        let str = keyStr + ': ';
    ...
    private codeFromTSModuleBlock(ast: TSModuleBlock): string {
        let str = '';
        for(let i = 0, len = ast.body.length; i < len; i++) {
            let bodyStr = this.codeFromAST(ast.body[i]);
            if(bodyStr) {
                if(str) {
                    str += '\n';
                }
    ...
    private startAddThis(reason: string) {
        this.addThisFlag += '.' + reason;
    }

    private stopAddThis(reason: string) {
        let r = '.' + reason;
    ...
    private getMemberInfo(ast: any, classInfo: ClassInfo, pname: string): PropertyInfo {
        ...
        while(classInfo.superClass) {
            let superClassFullName = classInfo.superClassFullName;
