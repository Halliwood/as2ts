...
　　　　　　　　　　// 读取文件内容
    ...
    // 读取ui json中的name和var字段进行保护
    var uiJsonDir = fs.readdirSync(uiJsonPath);        
    //遍历读取到的文件列表
    uiJsonDir.forEach(function(filename){
        //获取当前文件的绝对路径
        var filedir = path.join(uiJsonPath, filename);
        var extname = path.extname(filename).toLowerCase();
        if('.json' == extname) {
            var content = fs.readFileSync(filedir, 'utf-8');
            var fdata = JSON.parse(content);
            fetchUIJsonSaved(fdata);
        }
    });

    function fetchUIJsonSaved(fdata) {
        if(fdata['props']) {
            if(fdata['props']['var']) {
                noUglyMap[fdata['props']['var']] = true;
            }
            if(fdata['props']['name']) {
                noUglyMap[fdata['props']['name']] = true;
            }
        }
        if(fdata['child']) {
            for(let fc of fdata['child']) {
                fetchUIJsonSaved(fc);
            }
        }
    }
